    ...
    var body: some View {
        ZStack(alignment: .top) {
            VStack {
                Spacer()
                    .frame(height: 50)
                if let event = eventsModel.event(by: eventid), let item = eventsModel.item(by: event.itemid)  {
                ...
                VStack {
                    let uiState = dashboardEventsViewModel.getUIState(for: event, isPreview)
...
//                                eventsModel.events[0].state = uiState.primaryButtonActionState?.rawValue ?? EventState.archived.rawValue
//                            })
//                            .padding(10)
//                        }
//
...
//                                eventsModel.events[0].state = uiState.secondaryButtonActionState?.rawValue ?? EventState.archived.rawValue
//                            })
//                            .padding(10)
//                        }
//                    }
...
struct ActionItemView_Previews: PreviewProvider {
    static var previews: some View {
        ActionItemView(eventid: "", isPreview: true)
    }
}
